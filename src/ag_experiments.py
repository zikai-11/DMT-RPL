...
class NLI_infer_BERT(ModelWrapper):
    ...
    def forward_inference_whole_word_mask1(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        ...
        with torch.no_grad():
            ...
            logits = self.model.forward_infer(rebuild_ids1, mask_expand, seg_expand, inference=True)[0]

            logits = torch.softmax(logits,dim=-1)


            sentences = []
            for i ,(ids,ids1) in enumerate(zip(rebuild_ids1,input_ids_expand)):
                tokens = self.tokenizer.decode(ids, skip_special_tokens=True)
                tokens1 = self.tokenizer.decode(ids1)
                sentences.append((tokens1,tokens,logits[i][0],logits[i][1]))
    ...
    def __call__(self,text):
        ...
        with torch.no_grad():
            with open("sen.txt", 'w', encoding='utf-8') as file:
                probs = []
                for (ii,_) in enumerate(input_ids):
                    ...
                    for sentence in logits:
                        file.write(sentence[0] + ' ' + sentence[1] + ' ' + str(sentence[2].item()) + ' ' + str(sentence[3].item()) + '\n')
