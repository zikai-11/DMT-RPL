...
from src.Textattack.tests.test_tokenizers import question
...
_CONFIG = config_flags.DEFINE_config_file("config",default="experiments/configs/individual_llama2.py")
def main(_):

    pur = False
    params = _CONFIG.value

    gcg_path = "data/DeepInception.json"

    _MODELS = {
        "llama-2": ["llm_model/llama-2", {"use_fast": False}, "llama-2", 64]
    }
    ...
    for model in _MODELS:
        ...
        for idx , data in enumerate(log):
            ...
            for prompt in questions:
                ...
                if pur==True:
                    ...
                    for text_pur in purification_text:
                        ...
                        all_pur.extend(batch_outputs)
                    for gen_pur in all_pur:
                        jailbroken_pur = not any([prefix in gen_pur for prefix in _test_prefixes])
                        curr_jb_pur.append(jailbroken_pur)

                    if False in curr_jb_pur:
                        all_outputs.extend("I'm sorry")
                    else:
                        all_outputs.extend(all_pur[0])
            ...
            # clear cache
            del batch_inputs, batch_input_ids, batch_attention_mask, outputs, batch_outputs,model
            torch.cuda.empty_cache()
            #print(all_outputs[0])
            curr_jb, curr_em = [], []
