...
def main(_):
    ...
    _MODELS = {
        "Vicuna": ["../model/vicuna", {"use_fast": False}, "vicuna", 64]
    }
    ...
    ##读取防御后缀
    with open(params.defense_file, 'r') as f:
        log_defense = json.load(f)

    raw_def_controls = log_defense['def_controls']
    ...
    for model in _MODELS:
        ...
        for goal, target, control, def_control in zip(goals, targets, controls, ori_def_controls):
            print((
                f"\n====================================================\n"
                f"goal='{goal}'\n"
                f"target='{target}'\n"
                f"attack control='{control}'\n"
