...
_CONFIG = config_flags.DEFINE_config_file("config",default="experiments/configs/individual_vicuna.py")
def main(_):
    params = _CONFIG.value
    pur = True
    gcg_path = "data/benign/benign.csv"

    _MODELS = {
        "vicuna": ["llm_model/vicuna", {"use_fast": False}, "vicuna", 64]
    }

    with open(gcg_path, 'r', newline='', encoding='utf-8') as f:
        log = list(csv.reader(f))  # 将 csv.reader 的结果转换为列表

    #params.logfile = params.logfile.replace('results/', 'eval/')

    # 提取第一列（跳过标题行）
    prompts = [row[0] for i, row in enumerate(log) if i != 0]
    prompts = prompts[:50]
        ...
        for prompt in zip(prompts):
            ...
            if pur == True:
                ...
                for text_pur in purification_text:
                    text_pur = text_pur.replace("< s > ","").replace(") ","").replace(". ","").strip()
                    print((
                    f"\n====================================================\n"
                    f"text='{text_pur}'\n"
                    f"====================================================\n"
            ...
            for gen_str in all_outputs:
                ...
                print((
                f"\n====================================================\n"
                f"purication='{gen_str}'\n"
                f"purication='{jailbroken}'\n"
                f"====================================================\n"
                ))
            if False in curr_jb:
