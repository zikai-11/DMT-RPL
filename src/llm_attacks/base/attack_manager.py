...
class AttackPrompt(object):
    ...
    def target_loss(self, logits, ids):
        ...
        #输入是他俩都输入进去，然后让模型顺着sure here is 往下说，然后损失计算的是target前一个token 和整个target的损失
        return loss
...
class MultiPromptAttack(object):
    ...
    def run(self,
        ...
        for i in range(n_steps):
            ...
            if i % attack_freq == 0:
                control, loss = self.step( ##攻击生成的后缀control
        ...
        while step < n_steps:
            ...
            if num_goals < len(self.goals):
                ...
                if num_workers < len(self.workers):
                    ...
                    attack.log(step, n_steps, self.control, self.def_control, loss, def_loss, 0., model_tests,
                               verbose=verbose)
                    break
                else:
            ...
            for (mode, goals, targets) in zip(
                ...
                if control != prev_control and len(goals) > 0:
                    ...
                    for (gen_str, target) in zip(all_outputs, targets):
                        ...
                        #检查 gen_str 是否包含 test_prefixes 中的任意前缀（如果没有，jailbroken 为 True）。
                        # 检查 target 是否是 gen_str 的一部分（如果是，em 为 True）
