...
# #import plural
#
# from transformers import ElectraTokenizer, ElectraForPreTraining, AutoTokenizer
# import torch
...
class mask_ids:
    ...
    def __init__(self, cos_sim_file='cos_sim_counter_fitting.npy',
                 frequency_dict='frequency.json',threshold=200,syn_num = 50, syn_threshold = 0.5, ratio = 0.3, most_freq_num = 10):
    ...
    # def mask_frequency(self,tokenized,idx):
    #     complex_indexes = []
    #     list = []
    #     for idx1 in idx:
    #         word = tokenized[idx1]
    ...
    #         else:
    #             freq = 0
    #         list.append((idx1, freq))
    #     list.sort(key=lambda x: x[1], reverse=False)
    #     list = list[:int(len(list)*0.5)]
    #     for i in range(len(list)):
    #         complex_indexes.append(list[i][0])
    ...
    def mask_frequency(self, tokenized, idx):
        ...
        for idx1 in idx:
            ...
            if self.freq_dict.get(word) != None:
                ...
                freq = 0  # Default to 0 if word not found in freq_dict
            if freq < 200:
                complex_indexes.append(idx1)
    ...
    #     return perturb_words, perturb_indexes
    #
    # def get_synonyms(self, word, number=30):
